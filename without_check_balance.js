...
const getSolanaBalance = (fromKeypair) => new Promise(async (resolve) => {
    let success = false;
...
Status       : ${faucetStatus}`
            ...
            for (const [i, address] of randomAddresses.entries()) {
                ...
                const txstatus = await sendTransaction(transaction, keypair);
...
Points       : ${initialInfo.ring}
Mystery Box  : ${initialInfo.ring_monitor}
Status       : [${(i + 1)}/${randomAddresses.length}] ${txstatus} sent ${amountToSend} SOL to ${address}`
                });
